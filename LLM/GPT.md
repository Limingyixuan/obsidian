...
transformer结构优化困难，需要进行warm-up    Transformer结构对于warm-up的超参数（**持续轮数、增长方式、初始学习率**等）非常敏感，若调整不慎，往往会使得模型无法正常收敛。




| 对比维度       | Post-LN（后归一化）         | Pre-LN（前归一化）                 |
| ---------- | --------------------- | ---------------------------- |
| **梯度稳定性**  | 低层梯度指数衰减，深层易梯度消失 / 爆炸 | 梯度流动稳定，无明显梯度消失问题             |
| **训练难度**   | 训练不稳定，必须配合学习率 warmup  | 训练稳定，无需复杂预热机制                |
| **适用模型深度** | 仅适合浅层模型（<18 层）        | 支持深层 / 超大模型（百层以上）            |
| **正则化效果**  | 归一化覆盖融合后特征，正则化能力强     | 归一化仅作用于子层输入，正则化较弱            |
| **代表模型**   | 原始 Transformer（2017）  | BERT、GPT 系列、ViT、LLaMA 等现代大模型 |
| **结构特点**   | LayerNorm 在残差连接之后     | LayerNorm 在残差连接之前、子层计算之前     |
