...
csk算法在mosse算法的基础上，扩展了密集采样并核化相关滤波方法

kcf算法在csk的基础上采用了多通道梯度特征HOG

cn：基于多通道颜色特征：考虑颜色在目标追踪中的作用。

后续将CN和HOG方法结合，成为相关滤波目标追踪中最常用的方法。

颜色直方图

深度特征：用深度学习的方法代替手工特征提取方法，***UPDT之后将此算法与后边的深度学习的方法进行对比，看看两种算法的不同之处***，用深度特征来表示目标的高层特征，处理形变、旋转的问题，用手工特征来表示目标的纹理颜色信息。

#### 模型优化：

个人理解是对目标追踪的出现的各种问题进行针对性的解决，提出相应的模块对原有模型的缺点进行补足。

##### 核化方法：

提升模型的准确率和鲁棒性，如csk和kcf

##### 尺度估计的方法：

解决目标追踪过程中，检测目标的尺度大小会发生变化，所以检测框也要进行相应的缩小或者放大，如DSST和SAMF算法，***两者的具体区别还不清楚，回过头继续看***

##### 边界效应问题：

循环平移引起的边界效应问题，***循环平移的方法在相对滤波算法中使用，看完回来改*****

解决边界效应的方法：SRDCF，STRCF，DSARCF，CFLB
...
孪生网络是指具有相同或相似的两个并行网络。

个人理解属于深度学习方法的一类，**可以与后边的其他深度学习方法进行对比**

全卷积孪生网络SiameseFC

区域候选孪生网络SiameseRPN

注意力机制孪生网络RASNet，提出了通用注意力机制

基于时空关系的目标追踪，如Dsiam（目标形变转换机制和背景抑制转换机制）

无锚点孪生网络

深度学习：

MDnet开始，但是运行速度慢，无法实际使用，RT-MDNet (real-time MDNet)提升了运行速度
...
LMCF算法：判断跟踪结果是否准确，提出了APCE指标来判断是否发生了遮挡

SSRCF算法与上边的算法类似（马尔科夫决策——马尔科夫奖励——决策马尔科夫决策过程）

Siamese R-CNN 利用目标重检测机制解决跟踪中目标消失问题
