...
1. **`pred_logits`**
    ...
    - **形状**：`[batch_size, num_queries, num_classes]`
    - **通俗含义**：模型最终预测的**所有目标的类别得分**（未经过 softmax 归一化）。
        
        - `batch_size`：一次输入的图片数量
        - `num_queries`：模型预设的最大可检测目标数（比如 100，代表模型最多能预测 100 个目标）
        - `num_classes`：数据集的类别数（比如 VisDrone 的 10 类）
        
    - **作用**：后续通过 softmax 转成类别概率，判断每个预测框属于哪一类。
    
2. **`pred_boxes`**
    
    - **形状**：`[batch_size, num_queries, 4]`
    - **通俗含义**：模型最终预测的**所有目标的边界框坐标**，格式是 `(center_x, center_y, width, height)`。
        
        - 坐标值是**归一化到 0-1 之间**的（相对于图片自身尺寸，和图片大小无关）
        
    - **作用**：后续通过后处理器（`postprocessors`）还原成像素级坐标，得到最终的检测框位置。
    
3. **`loss_moe`**
    
    - **形状**：标量 Tensor（一个数字）
    - **通俗含义**：模型中 **MoE（混合专家模型）模块的平均损失**。
        
        - 如果模型没有 MoE 层，这个值就是 **0**
        - 如果有多个 MoE 层，会把所有层的损失加起来再取平均
        
    - **作用**：训练时参与总损失的计算，监督 MoE 模块的学习。
    
4. **`moe_loss_fn`**
    
    - **形状**：列表，列表中每个元素的形状是 `[n_dec, batch_size, num_global_experts]`
    - **通俗含义**：记录**每个 MoE 层的原始损失细节**。
        
        - 如果没有 MoE 层，这个列表就是空的
        
    - **作用**：用于调试 MoE 层，分析不同层的损失变化情况。
    
5. **`num_global_experts`**
    
    - **形状**：整数列表
    - **通俗含义**：记录**每个 MoE 层的专家数量**。
        
        - 如果没有 MoE 层，这个列表就是空的
        
    - **作用**：和 `moe_loss_fn` 配套使用，方便匹配 “专家数量” 和 “对应损失”。
    

二、 可选输出（满足特定条件才会存在）

这类输出是模型的扩展功能，只有开启对应配置才会出现。

1. **`aux_outputs`**
    
    - **存在条件**：`self.aux_loss = True`（开启辅助损失）
    - **形状**：列表，列表长度 = 解码器层数 - 1；列表中每个元素是字典 `{'pred_logits': ..., 'pred_boxes': ...}`
    - **通俗含义**：解码器**所有中间层**的预测结果（不是最后一层）。
    - **作用**：训练时计算**辅助损失**，让模型在训练初期就能学到准确的预测能力，提升最终精度（DETR 系列模型的核心技巧）。
    
2. **`interm_outputs`**
    
    - **存在条件**：`hs_enc is not None`（编码器有输出）
    - **形状**：字典 `{'pred_logits': [...], 'pred_boxes': [...]}`
    - **通俗含义**：编码器**最后一层**的预测结果。
    - **作用**：用于分析编码器的性能，或作为后续匹配任务的输入。
    
3. **`interm_outputs_for_matching_pre`**
    
    - **存在条件**：`hs_enc is not None`
    - **形状**：字典 `{'pred_logits': [...], 'pred_boxes': [...]}`
    - **通俗含义**：编码器最后一层的输出，专门用于**预处理阶段的目标匹配**。
    - **作用**：和 `interm_outputs` 的区别是，`pred_boxes` 用的是编码器的**初始提案框**，而非最终参考框。
    
4. **`enc_outputs`**
    
    - **存在条件**：`hs_enc.shape[0] > 1`（编码器有多层输出）
    - **形状**：列表，每个元素是字典 `{'pred_logits': ..., 'pred_boxes': ...}`
    - **通俗含义**：编码器**所有中间层**的预测结果。
    - **作用**：用于多层编码器的监督训练，或分析编码器各层的特征提取效果。
    
5. **`dn_meta`**
    - **存在条件**：`self.dn_number > 0` 或 `targets is not None`（开启去噪训练）
    - **形状**：字典（包含去噪训练的元信息）
    - **通俗含义**：记录去噪训练的关键参数，比如去噪查询数、标签噪声比例、框噪声尺度等。
    - **作用**：后续通过 `dn_post_process` 函数，对去噪训练的输出进行后处理，还原真实的检测结果。